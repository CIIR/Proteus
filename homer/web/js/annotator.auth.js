...
createDateFromISO8601 = function(string) {
  var d, date, offset, ref, regexp, time;
...
base64UrlDecode = function(data) {
  ...
  m = data.length % 4;
  if (m !== 0) {
    for (i = j = 0, ref = 4 - m; 0 <= ref ? j < ref : j > ref; i = 0 <= ref ? ++j : --j) {
      data += '=';
    }
  }
  data = data.replace(/-/g, '+');
...
parseToken = function(token) {
  var head, payload, ref, sig;
  ref = token.split('.'), head = ref[0], payload = ref[1], sig = ref[2];
  return JSON.parse(base64UrlDecode(payload));
};

Annotator.Plugin.Auth = (function(superClass) {
  ...
  Auth.prototype.setToken = function(token) {
    ...
    if (this.haveValidToken()) {
      ...
      while (this.waitingForToken.length > 0) {
        results.push(this.waitingForToken.pop()(this._unsafeToken));
      }
      return results;
    } else {
