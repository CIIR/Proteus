...
var WIKIPEDIA = function() {
    ...
    my.getData = function(wikipediaUrlOrPageName, callback, error) {
        ...
        // MCZ: some URLs redirect, so check for that.
        var r = new XMLHttpRequest();
        r.open("GET", url, true);
        //   r.overrideMimeType("text/xml");
        r.onload = function()
        {
            // make sure we point to a "resource" not a "page"
            if (!_.isUndefined(r.responseURL)) {
    ...
    my._expandNamespacePrefix = function(uriWithPrefix) {
        ...
        _.each(WIKIPEDIA.PREFIX, function(val, key) {
            if (uriWithPrefix.indexOf(key + ':') === 0) {
                uriWithPrefix = val + uriWithPrefix.slice(key.length + 1);
            }
        });
    ...
    my._lookupProperty = function(dict, property) {
        ...
        for (var i = 0; i < values.length; i++) {
            ...
            if (!values[i]['lang'] || values[i].lang == 'en') {
    ...
    my.extractSummary = function(subjectUri, rdfJson) {
        ...
        function lkup(attribs) {
            if (attribs instanceof Array) {
                ...
                for (var j = 0; j < attribs.length; j++) {
                    var _tmp = my._lookupProperty(properties, attribs[j]);
                    if (_tmp) {
                        out.push(_tmp);
                    }
                }
        ...
        if (_.isUndefined(typeObjs)) {
            typeObjs = [];

        }
        for (var k = 0; k < typeObjs.length; k++) {
            var value = typeObjs[k].value;
            // let's be selective
