...
public class GetResourcesForLabelsTest {
  ...
  private static boolean PARANOID = true; // set to true to verify existing functionality
  ...
  public void getResourcesTest() throws DBError {
    ...
    // test one label
    env.proteus.userdb.addTag(cred, "res1", "type1:value1", 5, null);
    ...
    // now add that label
    env.proteus.userdb.addTag(cred, "res1", "type1:value2", 5, null);
    // assuming existing functionality works.
    if (PARANOID) {
      List<String> res1tags = env.proteus.userdb.getTags(cred, "res1");
      Collections.sort(res1tags); // don't depend on db order
    ...
    env.proteus.userdb.addTag(cred, "res3", "type1:value1", 5, null);
    // assuming existing functionality works.
    if (PARANOID) {
    ...
    for (int i = 1; i <= 5; i++) {
      ...
      env.proteus.userdb.addTag(cred, "0-" + tmp, "type:0", 5, null);
      // assuming existing functionality works.
      if (PARANOID) {
    ...
    for (int i = 1; i <= 5; i++) {
      ...
      String tmp = Integer.toString(i);
      env.proteus.userdb.addTag(cred, "1-" + tmp, "type:1", i, null);
      // assuming existing functionality works.
      if (PARANOID) {
  ...
  // tests for when we want all resources ignoring the userid
  @Test
  public void getAllResourcesTest() throws DBError {
    String user = "maxdcat";
    env.proteus.userdb.register(user);
    Parameters p = env.proteus.userdb.login(user);
    ...
    env.proteus.userdb.addTag(cred, "res1", "type1:value1", 5, null);
    env.proteus.userdb.addTag(cred, "res2", "type1:value1", 5, null);
    env.proteus.userdb.addTag(cred, "res3", "type1:value2", 5, null);

    user = "newuser";
    env.proteus.userdb.register(user);
    p = env.proteus.userdb.login(user);

    cred = new Credentials(p);
    userid = cred.userid;

    env.proteus.userdb.addTag(cred, "resA", "type1:value1", 5, null);
    env.proteus.userdb.addTag(cred, "resB", "type1:value1", 5, null);
    env.proteus.userdb.addTag(cred, "resC", "type1:value2", 5, null);

    List<String> labels = new ArrayList<>();
    labels.add("type1:value1");
    List<String> resources = env.proteus.userdb.getResourcesForLabels(-1, labels);
    assertArrayEquals(new String[]{"res1", "res2", "resA", "resB"}, resources.toArray());
  }
}
